    ...
    for (const timeframe of resultArray) {
        ...
        if (!config && allCandles[timeframe][0] === undefined) {
            ...
            if (shouldMakeCandle(allCandles, timeframe, lastOneMinuteCandle, startTime, config)) {
                ...
                const { openPrice, high, low, closeTime } = calculateOHLC(lastOneMinuteCandle, addedTime, newV);
...
const calculateOHLC = (lastOneMinuteCandle, addedTime, newV) => {
    if (!newV && allCandles[timeframe][0] !== undefined) {
...
const updateCandles = (allCandles, timeframe, newCandle) => {
    ...
    if (existingCandleIndex >= 0) {
        ...
        if (allCandles[timeframe].length >= 3) {
            ...
            saveCandleDataToPostgreSQL(symbolName, timeframe, shouldSaveCandle).then(() => {
                if (allCandles[timeframe][1] !== undefined) {
                    allCandles[timeframe].splice(2, 1);
                }
            });
        }
    }
